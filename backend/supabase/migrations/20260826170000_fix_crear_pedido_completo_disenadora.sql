-- La migración 20260826161001_renombrar_diseniadora.sql renombró la columna
-- pedidos.id_diseñadora a id_disenadora, pero no actualizó esta función:
-- el rename de columna no reescribe el texto de los cuerpos plpgsql, así que
-- crear_pedido_completo() seguía insertando contra el nombre viejo de columna.
-- Esta migración reemplaza la función con el nombre de columna corregido
-- (el resto del cuerpo queda idéntico a 20260723143304_crear_pedido_completo.sql).
