-- modificar_plan_pedido ahora también guarda monto_cubierto al reinsertar las
-- cuotas. Además del corte por cuotas enteras que ya existía (floor), el
-- resto de esa división -- plata que ya entró pero no llega a cubrir una
-- cuota completa al valor nuevo -- se acredita como monto_cubierto en la
-- siguiente cuota, que pasa a estado 'Parcial' en vez de 'Pendiente'.
